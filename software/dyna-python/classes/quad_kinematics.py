...
from classes.algebra import RpToTrans, TransToRp, TransInv, RPY, TransformVector  
from collections import OrderedDict


class QuadModel:
    def __init__(self,
                 shoulder_length=0.0535,      # Old: 0.04 - New: 0.055       ours: 0.0515  # MEASURE (->)
                 elbow_length=0.10825,          # Old: 0.1 - New: 0.10652    ours: 0.10825
                 wrist_length=0.133,         # Old: 0.1 - New: 0.145         ours: 0.133
                 hip_x=0.238,           # Old: 0.192 - New: 0.23            ours: 0.2445
                 hip_y=0.0766,           # Old: 0.085 - New: 0.075           ours: 0.0766  # MEASURE (<-)
                 foot_x=0.24,#0.2445,          # Old: 0.192 - New: 0.23            # ADJUSTABLE (->)
                 foot_y=0.17,           # Old: 0.17 - New: 0.185
                 height=0.175,          # Old: 0.135 - New: 0.2  -- 0.155
                 com_offset=-0.03,     # Old:-0.018  - New: (calibarted with real spot)               # ADJUSTABLE (<-)
        ...
        self.Legs["FL"] = LegK("LEFT", self.shoulder_length,
                                self.elbow_length, self.wrist_length,
                                self.shoulder_lim, self.elbow_lim,
                                self.wrist_lim)
        self.Legs["FR"] = LegK("RIGHT", self.shoulder_length,
                                self.elbow_length, self.wrist_length,
                                self.shoulder_lim, self.elbow_lim,
                                self.wrist_lim)
        self.Legs["BL"] = LegK("LEFT", self.shoulder_length,
                                self.elbow_length, self.wrist_length,
                                self.shoulder_lim, self.elbow_lim,
                                self.wrist_lim)
        self.Legs["BR"] = LegK("RIGHT", self.shoulder_length,
                                self.elbow_length, self.wrist_length,
                                self.shoulder_lim, self.elbow_lim,
                                self.wrist_lim)
    ...
    def IK(self, orn, pos, T_bf):
        ...
        for i, (key, p_hf) in enumerate(HipToFoot.items()):
            ...
            joint_angles[i, :] = self.Legs[key].solveIK(p_hf)
    ...
    def FK(self, joint_angles):
        xyz = np.zeros((4,3))
        for index, lego in enumerate(self.Legs.values()):
            angles = joint_angles[index]
            xyz_coord = lego.solveFK(angles)
            xyz[index,:] = xyz_coord
        return xyz
