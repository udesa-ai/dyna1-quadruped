...
class LegK():
    def __init__(self,
                 legtype="RIGHT",
                 shoulder_length=0.0535,
                 elbow_length=0.10825,
                 wrist_length=0.133,  # Old: 0.1 - New:0.125
                 hip_lim=[-0.548, 0.548],
    ...
    def RightIK(self, x, y, z, D):
        ...
        joint_angles = np.array([shoulder_angle, -elbow_angle, -wrist_angle])  # np.array([-shoulder_angle, elbow_angle, wrist_angle])
        return joint_angles
    ...
    def solveFK(self, joint_angles):
        ...
        if self.legtype == "RIGHT":
            return self.RightFK(shoulder_angle, elbow_angle, wrist_angle)
        else:
            return self.LeftFK(shoulder_angle, elbow_angle, wrist_angle)
    
    def RightFK(self, shoulder_angle, elbow_angle, wrist_angle):
        x = self.elbow_length * np.sin(elbow_angle) + self.wrist_length * np.sin(elbow_angle + wrist_angle)
        y = (-self.wrist_length * np.sin(shoulder_angle) * np.cos(elbow_angle + wrist_angle)
             -self.shoulder_length * np.cos(shoulder_angle)
             -self.elbow_length * np.cos(elbow_angle) * np.sin(shoulder_angle))
        z = (self.shoulder_length * np.sin(shoulder_angle)
             -self.elbow_length * np.cos(shoulder_angle) * np.cos(elbow_angle)
             -self.wrist_length * np.cos(shoulder_angle) * np.cos(elbow_angle + wrist_angle))
        xyz_coord = np.array([x, y, z])
        return xyz_coord
    
    def LeftFK(self, shoulder_angle, elbow_angle, wrist_angle):
        x = self.elbow_length * np.sin(elbow_angle) + self.wrist_length * np.sin(elbow_angle + wrist_angle)
        y = (-self.wrist_length * np.sin(shoulder_angle) * np.cos(elbow_angle + wrist_angle)
             +self.shoulder_length * np.cos(shoulder_angle)
             -self.elbow_length * np.cos(elbow_angle) * np.sin(shoulder_angle))
